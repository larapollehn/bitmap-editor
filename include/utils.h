...
#define assert_equal(expected, actual, message, ...) if (expected != actual) { log_err(message,##__VA_ARGS__); exit(1); }
...
#define check_exit(A, M, ...) if(!(A)) {log_info(M, ##__VA_ARGS__); goto error_handling;}
