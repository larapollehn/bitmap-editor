...
void testCase18() {
    ...
    FILE * dest = fopen("field_m.bmp", "rb");
...
void testCase19(){
    ...
    // transform the bitmap image into an 24bpp image
    uint32_t transformed = Bitmap_transform(&bitmap);
    assert_equal(0, transformed, "Failed: Bitmap_transform");
